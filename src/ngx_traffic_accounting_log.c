...
ngx_int_t
ngx_traffic_accounting_log_metrics(void *val,
    ...
    ngx_log_t *log, char entry_n[], ngx_uint_t statuses[], ngx_uint_t statuses_len)
{
    ...
    created_at_sec = period->created_at_sec;
    updated_at_sec = period->updated_at_sec;
    ...
    if (nr_workers > 0) {
        p = ngx_slprintf(p, last,
            "workers:%ui|from:%i|to:%i|accounting_id:%V|%s:%ui|bytes_in:%ui|bytes_out:%ui|latency_ms:%ui|upstream_latency_ms:%ui",
            nr_workers,
            (ngx_int_t) created_at_sec, (ngx_int_t) updated_at_sec,
